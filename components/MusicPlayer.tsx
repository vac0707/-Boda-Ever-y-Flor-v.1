...
const MusicPlayer: React.FC<MusicPlayerProps> = ({ autoPlay = false }) => {
  ...
  return (
    <>
      ...
      <button
        onClick={toggleMusic}
        className="fixed bottom-6 right-6 z-[70] w-14 h-14 bg-white/95 backdrop-blur-md rounded-full shadow-2xl border-2 border-[#D4AF37] flex items-center justify-center text-[#2D6A4F] transition-all hover:scale-110 active:scale-95 group"
      ...
      >
        ...
        {isPlaying && (
          <span className="absolute inset-0 rounded-full border-2 border-[#2D6A4F] animate-ping opacity-30"></span>
        )}
      </button>
    </>
  );
